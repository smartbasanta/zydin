  ...
  <div>
    <DataTable>
      ...
      <template #add-new-button v-if="can.create">
        <RouterLink :to="{ name: 'dashboard.articles.create' }" class="btn btn-primary">
      ...
      <template #row="{ row }">
        ...
        <td v-if="visibleColumns.category" class="table-cell">
          <span class="inline-flex px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider rounded-md border bg-gray-50 text-gray-600 border-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700">
              {{ row.category }}
          </span>
        </td>
        
        <!-- Author -->
        <td v-if="visibleColumns.author" class="table-cell text-sm text-muted">
            {{ row.author || "N/A" }}
        </td>
        ...
        <td class="table-cell text-right">
          <div class="flex items-center justify-end gap-2">
            ...
            <EditButton 
                v-if="row.can?.update" 
                :to="{ name: 'dashboard.articles.edit', params: { id: row.id } }" 
                icon-only 
                size="sm" 
            />
            <DeleteModel 
      ...
      <template #empty-state>
        <td :colspan="articleColumns.filter(c => c.visible !== false).length + 1" class="text-center py-12">
          <div class="flex flex-col items-center justify-center text-muted">
            <div class="w-16 h-16 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mb-4">
                <BookOpenIcon class="size-8 text-gray-400" />
            </div>
            <h3 class="text-lg font-bold section-title mb-1">
              No Articles Found
            </h3>
            <p class="text-sm mb-6 max-w-xs mx-auto">
              Start publishing insights by adding your first article.
            </p>
            <RouterLink v-if="can.create" :to="{ name: 'dashboard.articles.create' }" class="btn btn-primary">
                <PlusIcon class="size-4 mr-2" />
                Add New Article
            </RouterLink>
          </div>
