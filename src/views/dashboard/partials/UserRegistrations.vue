...
defineProps<{
    stats: { title: string; value: string; change: string; icon: string; color: string; changeType: 'increase' | 'decrease' | 'neutral' }[];
}>();
</script>
...
<template>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div
            v-for="stat in stats"
            :key="stat.title"
            class="bg-white dark:bg-gray-1100 border border-gray-200 dark:border-gray-800/80 rounded-xl p-5 shadow-lg transition-all duration-300 hover:border-orange-600/50 dark:hover:border-orange-400/50 hover:shadow-orange-900/10 hover:-translate-y-1"
        >
